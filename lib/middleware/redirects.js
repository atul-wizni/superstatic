...
var Redirect = function(glob, pattern, destination, type) {
  this.type = type || 301;
  ...
  if (this.destination.match(/(?:^|\/):/)) {
    ...
    if (this.pattern) {
      this.engine = 'pattern';
      this.capture = patterns.createRaw(this.pattern);
    }
...
Redirect.prototype.test = function(url) {
  ...
  } else if (patterns.configMatcher(url, {'glob': this.glob, 'regex': this.pattern})) {
    return {
      type: this.type,
